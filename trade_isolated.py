...
def lets_make_some_money():
    for i in range(len(config.pair)):
        ...
        if api_binance.LONG_SIDE(response) == "LONGING":
            ...
            # print(unrealizedPnL)
            if unrealizedPnL > config.take_profit_percentage: api_binance.market_close_long(pair, response)
        ...
        if api_binance.SHORT_SIDE(response) == "SHORTING":
            ...
            # print(unrealizedPnL)
            if unrealizedPnL > config.take_profit_percentage: api_binance.market_close_short(pair, response)
